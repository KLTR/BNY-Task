...
describe('DataService', () => {
  ...
  describe('insertOrder', () => {
    const custId = 1;
    const order: IOrder[] = [{productName: 'Test Item', itemCost: 25.25}];
    it('should insert order to server', function () {
      service.inserOrder(custId, order).subscribe(res => {
  ...
  describe('getCustomers', () => {
    it('should return data', function () {
      service.getCustomers().subscribe(res => {
        expect(res).toEqual(customerList);
      });
    });
  });
});
